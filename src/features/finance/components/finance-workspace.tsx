...
export function FinanceWorkspace({initial}:{initial:FinanceSnapshot}) {
  ...
  return <div className="finance-workspace">
    <header className="finance-heading"><div><p className="eyebrow">Gestão financeira</p><h1>Financeiro</h1><p>Organize seus recebimentos, pagamentos e acompanhe o mês.</p></div>
      <label className="finance-period">Período<input type="month" min="1900-01" max="2100-12" value={filters.month} onChange={e=>{setFilters(current=>({...current,month:e.target.value,page:1}));setSelected(null);}}/></label>
    </header>
    ...
    {error ? <div role="alert" className="finance-empty"><h2>Financeiro indisponível</h2><p>{error}</p><button type="button" onClick={refresh}>Tentar novamente</button></div> : <>
      ...
      <div className="finance-columns">
        <section className="finance-ledger" aria-label="Lançamentos financeiros" aria-busy={loading}>
          ...
          <div className="finance-filters"><label>Buscar<input type="search" maxLength={150} value={filters.query} onChange={e=>setFilters(current=>({...current,query:e.target.value,page:1}))} placeholder="Descrição, pessoa ou categoria"/></label>
            <label>Situação<select value={filters.status} onChange={e=>setFilters(current=>({...current,status:e.target.value,page:1}))}><option value="all">Ativos</option><option value="open">Em aberto</option><option value="overdue">Em atraso</option><option value="partial">Baixa parcial</option><option value="settled">Pagos e recebidos</option><option value="cancelled">Cancelados</option></select></label></div>
          <p className="finance-list-count">{snapshot.count} lançamento(s) · vencimento em {snapshot.month.split("-").reverse().join("/")}. Filtros da lista não alteram o resumo mensal.</p>
          ...
          <nav className="finance-pagination" aria-label="Páginas de lançamentos"><button type="button" disabled={loading || filters.page<=1} onClick={()=>setFilters(current=>({...current,page:current.page-1}))}>Anterior</button><span>Página {snapshot.page} de {Math.max(1,Math.ceil(snapshot.count/30))}</span><button type="button" disabled={loading || filters.page*30>=snapshot.count} onClick={()=>setFilters(current=>({...current,page:current.page+1}))}>Próxima</button></nav>
