...
export function AcceptInviteForm({ token }: { token: string }) {
  ...
  return <form action={action} className="space-y-4"><input type="hidden" name="token" value={token} /><Feedback state={state} /><button disabled={pending} className="rounded-lg bg-accent px-4 py-3 text-white disabled:opacity-50">{pending ? "Entrando…" : "Aceitar convite e entrar na empresa"}</button></form>;
}
