...
const titles = { login: "Entrar na sua conta", signup: "Criar sua conta", recover: "Recuperar acesso", password: "Definir nova senha" };
export function AuthForm({ mode, invite = "", notice }: { mode: keyof typeof titles; invite?: string; notice?: string }) {
  ...
  return <section className="auth-content w-full max-w-md space-y-6">
    ...
    <form action={action} className="space-y-4 rounded-xl border border-border bg-surface p-6">
      ...
      {mode !== "recover" && <label className="block text-sm">Senha<input name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} required minLength={mode === "login" ? 1 : 8} maxLength={128} className="crm-input" />{mode !== "login" && <span className="mt-1 block text-xs text-[var(--ink-soft)]">Pelo menos 8 caracteres.</span>}</label>}
      ...
      <button disabled={pending} className="w-full rounded-lg bg-accent px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{pending ? "Aguarde…" : mode === "login" ? "Entrar" : mode === "signup" ? "Criar conta" : mode === "recover" ? "Enviar link de recuperação" : "Salvar nova senha"}</button>
