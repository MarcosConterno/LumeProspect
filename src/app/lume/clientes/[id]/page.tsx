...
import { ClientForm, RevokeInvite } from "@/features/platform/components/forms";
...
export default async function ClientPage({params}:{params:Promise<{id:string}>}) {
  ...
  return <div className="space-y-6"><header className="flex flex-wrap justify-between gap-4"><div><h1 className="font-display text-3xl">{workspace.name}</h1><p className="mt-2 text-sm">{workspace.is_lume?"Ambiente interno da Lume":"Configuração do cliente Lume"}</p></div><form action={selectWorkspace}><input type="hidden" name="workspace" value={id}/><button className="rounded-lg border border-border px-4 py-2 text-sm">Entrar neste ambiente</button></form></header>
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <section className="space-y-4 self-start rounded-xl border border-border bg-surface p-5"><h2 className="font-display text-xl">Usuários da empresa</h2><p className="text-sm text-[var(--ink-soft)]">Crie o acesso de uma pessoa com e-mail e senha, ou vincule uma conta existente.</p><Link href={"/lume/clientes/"+id+"/usuarios"} className="inline-block rounded-lg bg-accent px-4 py-3 text-sm font-semibold text-white">Criar usuário</Link><p className="text-xs">Para ajustar acessos e permissões, entre neste ambiente e abra Equipe. Convites estão disponíveis na página de criação de usuário.</p></section>
    </div>
