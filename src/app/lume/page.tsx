...
export default async function LumePage({searchParams}:{searchParams:Promise<{q?:string;page?:string;new?:string}>}) {
  ...
  return <div className="space-y-6">
    ...
    {params.new==="1"&&<section className="max-w-xl space-y-3 rounded-xl border border-border bg-surface p-5"><h2 className="font-display text-xl">Novo cliente Lume</h2><ClientForm/><p className="text-xs">Depois de cadastrar, selecione os módulos e use Criar usuário para cadastrar o acesso do administrador.</p></section>}
    ...
    <div className="grid gap-4 md:grid-cols-2">{rows.map(row=><article key={row.id} className="space-y-4 rounded-xl border border-border bg-surface p-5"><div><h2 className="font-display text-xl">{row.name}</h2><p className="mt-2 text-sm">{row.is_lume?"Empresa interna Lume":"Cliente Lume"} · {row.status==="active"?"Ativa":"Suspensa"}</p></div><div className="flex flex-wrap gap-3"><Link href={"/lume/clientes/"+row.id+"/usuarios"} className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white" aria-label={"Criar usuário em "+row.name}>Criar usuário</Link><Link href={"/lume/clientes/"+row.id} className="rounded-lg border border-border px-4 py-2 text-sm" aria-label={"Configurar "+row.name}>Configurar empresa</Link></div></article>)}</div>
